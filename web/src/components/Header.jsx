...
const TABS = ['HEATMAPS', '0DTE', 'OVERLAY', 'SCANNER', 'SWINGS', 'FLOW', 'NETFLOW', 'SWEEPS', 'BIGFLOW', 'SIGNALS', 'PORTFOLIO', 'SECTORS', 'HISTORY', 'MTF', 'EARNINGS', 'NEWS', 'GUIDE'];
const ICONS = {
  HEATMAPS: '🔥',
  '0DTE': '🎯',
  OVERLAY: '📈',
      ...
      {tab === 'HEATMAPS' && (
        <div className="header-controls">
          ...
          <div className="ctrl-group">
            ...
            <button className={`ctrl-btn ${viewMode === 'quad' ? 'active' : ''}`} onClick={() => setViewMode('quad')}>QUAD</button>
            <button className={`ctrl-btn ${viewMode === 'interval' ? 'active' : ''}`} onClick={() => setViewMode('interval')}>INTERVAL</button>
          </div>
          <button
