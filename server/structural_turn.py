...
CREATE TABLE IF NOT EXISTS structural_turns (
  ...
  tier TEXT,                             -- 'A' (7/7) | 'B' (6/7 regime fuzzy) | NULL
...
class StructuralTurnEvent:
    ...
    def core_5_passed(self) -> bool:
        """The 5 original gates — proximity, floor-event, volume, flow, NCP."""
        return all([
    ...
    @property
    def tier(self) -> str:
        """A: all 7 gates pass — auto-trade candidate.
           B: core 5 + magnitude pass; regime gate fuzzy (ambiguous middle zone) — watchlist.
           —: doesn't fire."""
    ...
    def to_row(self) -> dict[str, Any]:
        return {
            ...
            "tier": self.tier if self.tier in ("A", "B") else None,
...
def _gate_agg_flow(
    ...
    """Aggregate same-side flow notional in last AGG_FLOW_WINDOW_SEC."""
    ...
    if notional and notional >= AGG_FLOW_THRESHOLD:
        return True, f"agg {sentiment.lower()}-side flow ${notional/1e6:.1f}M ({count} alerts)", \
               {"notional": notional, "count": count}
    return False, f"agg flow ${notional/1e6:.1f}M < ${AGG_FLOW_THRESHOLD/1e6:.0f}M threshold", \
...
async def _send_structural_turn_telegram(ev: StructuralTurnEvent) -> None:
    ...
    tier_emoji = "⚡" if ev.tier == "A" else "👁"
    tier_label = (
        "TIER A — auto-trade candidate (all 7 gates)"
        if ev.tier == "A"
        else "TIER B — watchlist (regime ambiguous, 6/7 gates)"
    )
    ...
    lines.append("")
    if ev.tier == "A":
        lines.append("Play (BULL): ATM 0DTE call. Size half. "
                     "Trail stop at +100% peak; full exit at EOD or king.")
    else:
        lines.append("Play (BULL): smaller size. Validate against chart "
                     "(volume? MA reclaim?). Skip if discretionary read disagrees.")
    if ev.direction == "BEARISH":
        lines[-1] = lines[-1].replace("call", "put").replace("king", "floor")
    lines.append("")
    ...
    try:
        ...
        conn.execute(
            """INSERT OR IGNORE INTO structural_turns (
                 ...
                 gate_magnitude, gate_regime_match, tier,
            ...
            (row["ticker"], row["ts"], row["iso"], row["direction"],
             ...
             row["gate_magnitude"], row["gate_regime_match"], row["tier"],
