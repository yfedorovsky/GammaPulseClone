...
- **⭐ NEXT — ThetaData Python client (replaces the Java Terminal).** ThetaData now ships a Python client/SDK, so the integration no longer needs the local Java Terminal (`thetadata.exe`, v3 REST at `127.0.0.1:25503`). Opportunity: simplify + likely unblock the stock spot poller (Fix 3) + maybe kill the "current day" request quirk. Audit `server/thetadata.py`, `theta_v3_query.py`, `alert_outcomes.fetch_option_nbbo_bars`, the OPRA stream (`sweep_detector`) first; confirm the client's parity (stream + snapshot + history + PRO auth) and headless support before refactoring.
